...
from IOAM.test_data_viewer import data_viewer
...
class Waveglider(object):
    ...
    def render(self):
        data_viewer(self.x1, self.y1, u1=self.u1, phit=self.phi1, rudder_angle=self.Rudder_angle, t=self._t, xlim_left=self.xlim_left, xlim_right=self.xlim_right, ylim_left=self.ylim_left, ylim_right=self.ylim_right,
                        goal_x=self.target_position[0], goal_y=self.target_position[1], obs1=self.obstacle_1, R1=self.obst_R1, obs2=self.obstacle_2, R2=self.obst_R2, obs3=self.obstacle_3, R3=self.obst_R3,obs4=self.obstacle_4, R4=self.obst_R4,obs5=self.obstacle_5, R5=self.obst_R5)
...
new_saver.restore(sess, tf.train.latest_checkpoint('D:/Wave glider modelling/data/#9'))
