...
from Model.WG_dynamics import WG_dynamics
...
def simulation():
    ...
    for t in simulation_time(terminal_time=500):
        ...
        if state_0.item(11) > pi:
            ...
            state_0[11] = state_0.item(11) + 2*pi
        '''
        print(state_0.item(12), state_0.item(13))

        # interval of refreshing the monitor, default: 2s
        if t % 2 == 0:
            data_viewer(x1, y1, u1, phit, Rudder_angle, _t,
                        xlim_left=x_left, xlim_right=x_right, ylim_left=y_left, ylim_right=y_right,
                        goal_x=goal_x, goal_y=goal_y)  # T=T, Ffoil_x=Ffoil_x
